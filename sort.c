...
//                 // If the bit is not set,
	perform action A (rotate) instead of B (push to B)
//                 ra(*a);
//             } else {
//                 // If the bit is set,
	perform action B (push to B) instead of A (rotate)
//                 pb(a, b);
//             }
//         }

//         // Move elements back from B to A,
	ensuring those with bit 0 are now at the bottom
//         while (!is_empty(*b)) {
//             pa(a, b);
//         }
//     }
...
void	sort_stack(t_stack **a, t_stack **b)
{
	...
	while (bit < maxBits && !is_sorted(*a))
	{
		...
		// Move elements back from B to A,
			ensuring those with bit 1 are now at the bottom
		while (!is_empty(*b))
		{
			pa(a, b);
		}
